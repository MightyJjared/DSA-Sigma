...
int knapsack(int n, int W, vector<int> &wt, vector<int> &val, vector<vector<int>> &dp){
    ...
    for(int i = wt[0]; i <= W; i++){ //base case
        dp[0][i] = val[0];
    }
    for(int index = 1; index < n; index++){
        for(int k =0; k <= W; k++){
            int not_pick = 0 + dp[index-1][k];

            int pick = INT_MIN;
            if(wt[index] <= k){
                pick = val[index] + dp[index-1][k - wt[index]];
            }

            dp[index][k] = max(pick, not_pick);
        }
    }
...
int main() {
    ...
    cout << "Maximum value in Knapsack = " << knapsack(n, W, wt, val, dp) << endl;
